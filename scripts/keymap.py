...
def expand_modifier_chords(model: dict[str, Any]) -> None:
    ...
    for family in source.get("modifier_chords", []):
        ...
        for count in range(1, len(modifiers) + 1):
            for selected in itertools.combinations(enumerate(modifiers), count):
                ...
                behaviors[name] = {
                    ...
                    "modifier_positions": [item["position"] for item in modifiers],
                    "activation_layers": [*family["layers"], family["layer"]],
                    "trigger_modifiers": sum(1 << index for index in selected_indices),
                    "combo_term_ms": family["term_ms"],
                }
                combo = {
                    "name": name,
                    "positions": [family["layer_position"], *(item["position"] for item in selected_modifiers)],
                    "layers": [*family["layers"], family["layer"]],
                    "action": {"use": name},
...
def validate_action(model: dict[str, Any], value: Any, context: str) -> None:
    ...
    if "tap" in value and "adaptive" in value and "hold" not in value:
        ...
        if value["adaptive"] not in model["behaviors"].get("adaptives", {}):
...
def validate(model: dict[str, Any]) -> None:
    ...
    for name, behavior in behaviors.items():
        ...
        if behavior["recipe"] == "contextual_key":
            if not valid_key(model, behavior.get("default", "")) or not isinstance(behavior.get("label"), str) or not behavior["label"]:
                fail(f"behavior {name}: invalid contextual key default or label")
            if behavior.get("timeout_ms", 0) <= 0 or not isinstance(behavior.get("strict_modifiers"), bool):
                fail(f"behavior {name}: invalid contextual key settings")
            for rule in behavior.get("rules", []):
                after = rule.get("after")
                after_any = rule.get("after_any")
                if (after is None) == (after_any is None):
                    fail(f"behavior {name}: contextual rule needs exactly one of after or after_any")
                triggers = after if after is not None else after_any
                if not isinstance(triggers, list) or not 1 <= len(triggers) <= 32 or any(not valid_key(model, token) for token in triggers):
                    fail(f"behavior {name}: invalid contextual rule triggers")
                emit = rule.get("emit", [])
                if not isinstance(emit, list) or not 1 <= len(emit) <= 6 or any(not valid_key(model, token) for token in emit):
                    fail(f"behavior {name}: invalid contextual rule output")
                if rule.get("timeout_ms", behavior["timeout_ms"]) <= 0 or not isinstance(rule.get("strict_modifiers", behavior["strict_modifiers"]), bool):
                    fail(f"behavior {name}: invalid contextual rule settings")
        ...
        if behavior["recipe"] == "layer_sticky_mod":
            ...
            if not activation_layers or not set(activation_layers).issubset(declared_layers):
                fail(f"behavior {name}: invalid activation layers")
        ...
        if behavior["recipe"] == "repeat_magic":
            ...
            repeat_keys = behavior.get("repeat_keys", [])
            if not isinstance(repeat_keys, list) or len(repeat_keys) != len(set(repeat_keys)) or any(not valid_key(model, key) for key in repeat_keys):
                fail(f"behavior {name}: invalid repeat keys")
    ...
    for index, conditional in enumerate(conditional_layers):
        ...
        signature = (then_layer, tuple(sorted(variants)))
        if signature in seen_conditional_layers:
            fail(f"conditional layer {index}: duplicate condition")
        seen_conditional_layers.add(signature)
    ...
    for name, profile in profiles.items():
        ...
        if not isinstance(profile.get("extended_thumbs", False), bool):
            fail(f"profile {name}: extended_thumbs must be boolean")
        for backend in ("zmk", "qmk"):
            ...
            if profile.get("extended_thumbs") and not {"L_THUMB_OPTIONAL", "R_THUMB_OPTIONAL"}.issubset(set(slots) - set(board_only)):
                fail(f"profile {name}: extended_thumbs needs six physical thumb positions")
    ...
    for index, combo in enumerate(behavior_source.get("combos", [])):
        ...
        if "feature_value" in combo and (feature is None or not isinstance(combo["feature_value"], bool)):
            fail(f"combo {name}: invalid feature value")
        ...
        if "center" in drawing and (not isinstance(drawing["center"], int) or not 0 <= drawing["center"] <= 4):
            fail(f"combo {combo['name']}: invalid draw center")
        if "name" in drawing and (not isinstance(drawing["name"], str) or not drawing["name"]):
            fail(f"combo {combo['name']}: invalid draw name")
...
def feature_enabled(model: dict[str, Any], name: str, variant: str) -> bool:
    value = model["root"]["features"][name]
...
def zmk_action(model: dict[str, Any], ir: dict[str, Any], value: Any, adaptive_name: str | None = None) -> str:
    ...
    if "use" in value:
        ...
        if recipe == "sticky_key":
            ...
            return f"&{name} {zmk_key(model, key)} {'0' if key in {'LSHFT', 'RSHFT'} else zmk_key(model, key)}"
...
def render_zmk_behaviors(model: dict[str, Any], ir: dict[str, Any]) -> list[str]:
    ...
    immediate_hold = "; hold-while-undecided" if home_row_mods and h.get("hold_while_undecided", False) else ""
    flavor = h["flavor"] if home_row_mods else "tap-preferred"
    lines = [
    ...
    for name, item in model["behaviors"]["behaviors"].items():
        ...
        if recipe == "shift_morph":
            ...
            for index, rule in enumerate(item["rules"]):
                after = rule.get("after")
                triggers = rule["after_any"] if "after_any" in rule else [after[-1]]
                properties = [f"trigger-keys = <{' '.join(zmk_key(model, token) for token in triggers)}>;"]
                if after is not None and len(after) > 1:
                    properties.append(f"prior-keys = <{' '.join(zmk_key(model, token) for token in after[:-1])}>;")
                properties.append(f"max-prior-idle-ms = <{rule.get('timeout_ms', item['timeout_ms'])}>;")
                properties.append(f"bindings = <{' '.join(f'&kp {zmk_key(model, token)}' for token in rule['emit'])}>;")
                if rule.get("strict_modifiers", item["strict_modifiers"]):
            ...
            modifier = resolved_key(model, item["key"])
            tap_binding = "&smart_shift" if modifier in {"LSHFT", "RSHFT"} else "&sk"
            lines.append(f'ZMK_HOLD_TAP({name}, bindings = <&kp>, <{tap_binding}>; flavor = "{timing["flavor"]}"; tapping-term-ms = <{timing["tapping_term_ms"]}>; hold-while-undecided; hold-while-undecided-linger;)')
    ...
    if layer_chords or ir["layer_stacks"] or layer_sticky_mods:
        ...
        for name, item in layer_sticky_mods:
            ...
            lines.extend([
                ...
                f'            sticky-bindings = {", ".join(sticky_bindings)};',
                f'            tapping-term-ms = <{timings["layer_modifier"]["tapping_term_ms"]}>;',
                "        };",
            ])
        lines.extend(["    };", "};", ""])
    magic = behavior(model, "thumb_magic")
    alpha_keys = sorted(zmk_key(model, chr(value)) for value in range(ord("A"), ord("Z") + 1))
    nonalpha_keys = sorted({zmk_key(model, key) for key in KEYS - MODS} - set(alpha_keys))
    repeat_rules = [
        f"repeat_alpha {{ trigger-keys = <{' '.join(alpha_keys)}>; bindings = <&alpha_repeat>; max-prior-idle-ms = <{magic['repeat_timeout_ms']}>; }};"
    ]
    repeat_keys = " ".join(zmk_key(model, key) for key in magic.get("repeat_keys", []))
    if repeat_keys:
        repeat_rules.append(
            f"repeat_plain {{ trigger-keys = <{repeat_keys}>; bindings = <&alpha_repeat>; max-prior-idle-ms = <{magic['repeat_timeout_ms']}>; }};"
        )
    for modifier, wrapper in ZMK_MODS.items():
        for index in range(0, len(nonalpha_keys), 32):
            keys = " ".join(f"{wrapper}({key})" for key in nonalpha_keys[index:index + 32])
            repeat_rules.append(
                f"repeat_{modifier.lower()}_{index // 32} {{ trigger-keys = <{keys}>; bindings = <&alpha_repeat>; max-prior-idle-ms = <{magic['repeat_timeout_ms']}>; }};"
            )
    repeat_triggers = " ".join(repeat_rules)
    lines.extend([
        '/ { behaviors { alpha_repeat: alpha_repeat { compatible = "zmk,behavior-alpha-repeat"; #binding-cells = <0>; }; }; };',
        f"ZMK_ADAPTIVE_KEY(thumb_magic_adaptive, bindings = <&sk LSHFT>; {repeat_triggers})",
    ...
    for adaptive_name in model["behaviors"].get("adaptives", {}):
        ...
        for input_key, input_rules in sorted(by_input.items()):
            ...
            for hand in sorted({cell.get("hand") for cell in used_cells if isinstance(cell, dict) and cell.get("hand")}):
                ...
                trigger = f'; hold-trigger-key-positions = <{" ".join(map(str, positions))}>; hold-trigger-on-release' if home_row_mods and h["opposite_hand_hold"] else ""
                lines.append(f'ZMK_HOLD_TAP({base}_{side}, bindings = <&kp>, <&{base}>; flavor = "{flavor}"; tapping-term-ms = <{h["tapping_term_ms"]}>; quick-tap-ms = <{h["quick_tap_ms"]}>{prior_idle}{trigger};)')
    return lines
...
def tap_dance_spec(model: dict[str, Any], value: Any) -> dict[str, Any] | None:
    ...
    if isinstance(value, dict) and "use" in value:
        ...
        if item["recipe"] == "tap_hold":
            ...
            return {
                "name": f"sticky_{key}",
                "tap_kind": "RAZEN_TAP_SMART_SHIFT" if key in {"LSHFT", "RSHFT"} else "RAZEN_TAP_ONESHOT_MOD",
...
def render_qmk(model: dict[str, Any], ir: dict[str, Any]) -> str:
    ...
    for name, item in model["behaviors"]["behaviors"].items():
        ...
        matching_layers = [
            layer_name
            for layer_name, cells in ir["layers"].items()
            if any(isinstance(cell, dict) and cell.get("use") == name for cell in cells)
        ]
        rules = []
        for rule in item["rules"]:
            suffixes = [rule["after"]] if "after" in rule else [[token] for token in rule["after_any"]]
            for after in suffixes:
                rules.append((after, rule))
        for matching_layer in matching_layers:
            for after, rule in sorted(rules, key=lambda entry: -len(entry[0])):
                after_keys = [qmk_key(model, token) for token in after]
                emit = [qmk_key(model, token) for token in rule["emit"]]
                timeout = rule.get("timeout_ms", item["timeout_ms"])
                strict = rule.get("strict_modifiers", item["strict_modifiers"])
                lines.append(f"    {{{qmk_layer(matching_layer)}, {qmk_key(model, item['default'])}, {{{', '.join(after_keys)}}}, {len(after_keys)}, {{{', '.join(emit)}}}, {len(emit)}, {timeout}, {'true' if strict else 'false'}}},")
                adaptive_count += 1
    ...
    for spec in native_tap_holds:
        ...
        if timing.get("quick_tap_ms"):
            quick_taps[spec["keycode"]] = timing["quick_tap_ms"]
    lines.extend([
        "};",
        ...
        "",
        "const razen_quick_tap_t razen_quick_taps[] = {",
        *(f"    {{{keycode}, {term_ms}}}," for keycode, term_ms in quick_taps.items()),
        "};",
        "const uint8_t razen_quick_tap_count = sizeof(razen_quick_taps) / sizeof(razen_quick_taps[0]);",
        "",
...
def render_qmk_config(model: dict[str, Any], ir: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *(
        ...
        f"#define RAZEN_MAGIC_PLAIN_REPEATABLE(keycode) ({' || '.join(f'(keycode) == {qmk_key(model, key)}' for key in magic.get('repeat_keys', [])) or 'false'})",
        *(["#define RAZEN_HOME_ROW_MODS", "#define CHORDAL_HOLD"] if home_row_mods else []),
        ...
        *([f"#define FLOW_TAP_TERM {timing['home_row']['prior_idle_ms']}"] if home_row_mods else []),
...
def label_action(model: dict[str, Any], ir: dict[str, Any], value: Any) -> Any:
    ...
    if "tap" in value:
        if "hold" not in value:
            return label_action(model, ir, value["tap"])
        hold = value["hold"]
...
def render_draw(model: dict[str, Any], ir: dict[str, Any]) -> str:
    ...
    for source, cells in ir["layers"].items():
        ...
        for index, cell in enumerate(cells):
            ...
            types = str(styled.get("type", "")).split()
            activator = f"{ident(target).lower()}-activator"
            styled["type"] = " ".join(dict.fromkeys((*types, activator)))
    ...
    for alpha in model["root"]["alpha_layers"]:
        for index, cell in enumerate(ir["layers"].get(alpha, [])):
            target = held_layer(model, cell)
            if target:
                activators.setdefault(target, set()).add(index)
    for conditional in ir["conditional_layers"]:
        sources = conditional["if_layers"]
        for source in sources:
            if source not in layers:
                continue
            for index, rendered in enumerate(layers[source]):
                if isinstance(rendered, dict) and rendered.get("type") == "held" and index not in activators.get(source, set()):
    ...
    for combo in ir["combos"]:
        ...
        if "center" in drawing:
            trigger_type = f"{drawing['type']}-trigger"
            for layer in draw_layers:
                for index in combo["indices"]:
                    trigger = layers[layer][index]
                    ...
                    if any(value.startswith("meta-") and value.endswith("-trigger") for value in trigger_types):
                        ...
                        trigger["type"] = " ".join((*trigger_types, trigger_type))
                        layers[layer][index] = trigger
                centered_key = dict(key) if isinstance(key, dict) else {"t": key}
                centered_key["type"] = f"{centered_key.get('type', '')} meta-combo".strip()
                centered_key["h"] = drawing["name"]
                combos.append({
                    "p": center_positions,
                    "k": centered_key,
                    "l": [layer],
                    "a": "bottom",
                    "o": -0.6 + drawing["center"] * 0.62,
                    "d": False,
                    "w": 74,
                    "h": 27,
                })
...
def render_svg(repo: Path, ir: dict[str, Any], yaml_path: Path, svg_path: Path, layers: list[str] | None = None, labels: dict[str, str] | None = None) -> None:
    ...
    svg = glyph_pattern.sub("", svg)
    if layers:
        svg = svg.replace('class="keymap"', 'class="keymap layer-summary"', 1)
    for source, target in (labels or {}).items():
        svg = svg.replace(f">{source}</text>", f">{target}</text>")
...
def generate_one(model: dict[str, Any], repo: Path, backend: str, profile: str, os_name: str, output: Path | None, svg: bool) -> list[Path]:
    ...
    if backend == "draw" and svg:
        ...
        if layers := profile_data.get("draw_summary_layers"):
            summary_path = directory / "keymap_summary.svg" if output else repo / "draw" / "generated" / f"{profile}_summary.svg"
            render_svg(repo, ir, directory / "keymap.yaml", summary_path, layers, profile_data.get("draw_summary_labels"))
